"""
Architecture from https://github.com/s-sahoo/duo/blob/main/models/dit.py.

Current edits:
    - Remove softmax from embedding layer;
    - Move config in constructor;
...
class EmbeddingLayer(nn.Module):
  ...
  def forward(self, x):
    ...
    # TODO: we might have inputs that are not on the simplex. The following operation is legal,
    # but is it fine? (Should be?)
    # non softmax-ed version (assuming the inputs are on the simplex):
    return torch.einsum(
      "blv,ve->ble",
      x.float(),
